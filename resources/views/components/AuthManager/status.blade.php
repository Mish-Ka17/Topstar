<div class="w-[10%]">
    @isset($user)
      <el-dropdown class="inline-block">
        <button class="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white/10 px-3 py-2 text-sm font-semibold text-gray-600 hover:bg-white/20 cursor-pointer"
        title="{{$user->name}}">
        {{$user->name}}
        ...
        <el-menu anchor="bottom end" popover class="origin-top-right rounded-md bg-gray-100 outline-1 -outline-offset-1 outline-white/10 transition transition-discrete [--anchor-gap:--spacing(2)] data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in">
